...
public extension Date {
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        let timestampValue = timestamp.count == 10 ? Double(timestamp) : (Double(timestamp) ?? 0) / 1000
        // 时间戳转为Date
...
public extension Date {
    ...
    func callTimeAfterNow() -> String {
        ...
        if timeInterval < 0 {
            return NSLocalizedString("just_now", comment: "just_now")
        }
        ...
        if i3600 < 1 {
            ...
            if s == 0 {
                time = NSLocalizedString("just_now", comment: "just_now")
            } else {
                time = "\(s)" + NSLocalizedString("mins_ago", comment: "mins_ago")
            }
        } else if i86400 < 1 {
            let s = NSNumber(value: i3600 as Double).intValue
            time = "\(s)" + NSLocalizedString("hours_ago", comment: "hours_ago")
        } else if i2592000 < 1 {
            let s = NSNumber(value: i86400 as Double).intValue
            time = "\(s)" + NSLocalizedString("days_ago", comment: "days_ago")
        } else if i31104000 < 1 {
            let s = NSNumber(value: i2592000 as Double).intValue
            time = "\(s)" + NSLocalizedString("months_ago", comment: "months_ago")
        } else {
            let s = NSNumber(value: i31104000 as Double).intValue
            time = "\(s)" + NSLocalizedString("years_ago", comment: "years_ago")
        }
        return time
    }
}
